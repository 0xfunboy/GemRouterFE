...
export const CHATGPT_MCP_OUTPUT_SCHEMAS = {
  open: z.object({
    ...
    limits: z.object({ default_wait_seconds: z.number(), maximum_wait_seconds: z.number(), maximum_response_bytes: z.number() }).strict(),
  }).strict(),
  exchange: z.object({
    protocol_version: z.literal(CHATGPT_GATEWAY_PROTOCOL_VERSION), next_exchange_id: z.string(), continue: z.boolean(),
    state: z.enum(['request', 'idle', 'recovery', 'released']),
...
Keep run_id, exchange_id, request_id and claim_token inside gateway calls. Copy server handles exactly. A completion contains response text or a structured error, never both. Discard late results after cancellation, expiry or stale-claim recovery. The server cannot wake this chat or guarantee an infinite loop; stop after persistent transport/protocol failures and ask the operator to reconnect or wake the dedicated conversation.`;
...
export function createChatGptMcpServer(gateway: ChatGptGateway, grant: AuthenticatedMcpGrant, sessionSignal?: AbortSignal): McpServer {
  ...
  server.registerTool('gateway_exchange', {
    ...
    description: 'Atomically submit the preceding completion and enter the next bounded poll. Reuse identical run/exchange arguments after an ambiguous transport timeout; the whole exchange is replayed without advancing twice.',
    inputSchema: z.object({
