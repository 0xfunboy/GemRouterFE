...
readline.createInterface({ input: process.stdin }).on('line', (line) => {
  ...
  } else if (message.method === 'account/read') reply({ account: { type: 'chatgpt', email: 'fixture@example.test', planType: 'pro' } });
  ...
  else if (message.method === 'account/rateLimits/read') reply({ rateLimits: { limitId: 'codex', primary: { usedPercent: 25, windowDurationMins: 300, resetsAt: Math.floor(Date.now()/1000)+3600 } } });
