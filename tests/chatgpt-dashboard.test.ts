...
describe('ChatGPT gateway dashboard integration', () => {
  ...
  it('provides a gated accessible three-step wizard with honest connection status and safe recovery', () => {
    const html = renderAppShell({ projectName: 'GemRouter', modelIds: [], publicBaseUrl: 'https://gemrouter.example.com' });
  ...
  it('only continues login to an exact same-origin OAuth authorization path', () => {
    ...
    const check = (value: string) => vm.runInContext(`chatGptAuthorizationContinuation(${JSON.stringify('?chatgpt_authorize=' + encodeURIComponent(value))}, 'https://gemrouter.example.com')`, context);
