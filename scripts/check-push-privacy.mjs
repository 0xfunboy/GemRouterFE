...
const refs = process.argv.slice(2).length ? process.argv.slice(2) : ['HEAD'];
...
const patterns = [
  ...
  ['personal-email', /[\w.+-]+@(?:gmail|googlemail|outlook|hotmail|yahoo)\.[a-z.]+/i],
...
for (const commit of commits) {
  ...
  for (const row of git('ls-tree', '-r', '-z', commit).split('\0').filter(Boolean)) {
    ...
    if (type !== 'blob' || scanned.has(id)) continue;
    scanned.add(id);
...
console.log(JSON.stringify({ outgoingCommits: commits.length, scannedBlobs: scanned.size, findings }, null, 2));
