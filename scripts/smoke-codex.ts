  ...
  try {
    ...
    if (!options.live) {
      ...
      if (options.ui) {
        ...
        assert.equal(await page.locator('#codex-account-panel').evaluate((el: HTMLDetailsElement) => el.open), false);
        await page.locator('#codex-account-panel summary').first().click();
        await page.getByRole('button', { name: 'Verifica account', exact: true }).click();
        await page.getByText('Operazione conclusa. Nessuna inferenza avviata.').waitFor();
