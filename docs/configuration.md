...
Optional and disabled by default. See [Codex setup, API and quotas](codex-account.md)
for the complete configuration. Set `GEMROUTER_CODEX_ENABLED=true` only during an
authorized deployment, then explicitly enable Codex per app. Supported IDs are
`gpt-5.5`, `gpt-5.6-luna`, `gpt-5.6-sol`, `gpt-6-astra`; model-specific thinking
is validated from the authenticated account's catalog. Existing apps remain disabled.

GPT requests use Codex regardless of ordinary backend ordering. Only exhausted
Codex quota may fall back to the first authorized model in the configured Gemini
chain. No unrelated Gemini/NVIDIA request spills into Codex. An explicit backend
pin disables fallback. Missing quota data stays unknown; quota percentage is not
converted into a fabricated token or request budget.

The old MCP, widget and personal-chat controller flags no longer register routes.
Their code and verification history are preserved in the [archive](chatgpt-widget-wake-verification.md).
Private Codex login is reused, and old MCP grants/databases are left untouched.
