...
Current official guidance describes **Settings → Security and login → Developer mode**, then **Plugins → +**, a user-facing name and description, and a public HTTPS MCP server URL. OpenAI's OAuth documentation confirms protected-resource discovery, DCR, authorization code with PKCE S256, and scope discovery. Labels and availability can still vary by account/workspace. These ChatGPT-side confirmations and the conversation start cannot be automated by this server. See [OpenAI's connection instructions](https://developers.openai.com/plugins/deploy/connect-chatgpt) and [OAuth requirements](https://developers.openai.com/plugins/build/auth). The OpenAI Docs review informed the wizard's steps and its explicit automation limits.
