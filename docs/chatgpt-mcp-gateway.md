...
GEMROUTER_CHATGPT_PUBLIC_BASE_URL=https://gemrouter.example.com
...
Sign in at [GemRouter administration](https://gemrouter.example.com/admin) and open **ChatGPT MCP Gateway**. The Italian-language wizard provides three steps:
...
2. **Autorizza**: copy the generated values and use **Apri ChatGPT**. The wizard follows the current **New Plugin** form from top to bottom: optional icon, suggested name, suggested description, **Connection → Server URL** (not Tunnel), generated MCP URL, then **Authentication → OAuth**. Under **Advanced OAuth settings**, choose **Client registration → Dynamic Client Registration (DCR)**; the CIMD-unavailable notice is expected because this gateway deliberately advertises DCR. Under **Scopes**, keep `mcp:tools` and `offline_access` selected as default scopes and leave **Base scopes** empty. Accept **I understand and want to continue**, choose **Create**, then sign in to GemRouter if requested and approve only the expected worker. The wizard polls the server and only unlocks the next step when a non-revoked grant exists.
...
Current official guidance describes **Settings → Security and login → Developer mode**, then **Plugins → +**, a user-facing name and description, and a public HTTPS MCP server URL. OpenAI's OAuth documentation confirms protected-resource discovery, DCR, authorization code with PKCE S256, and scope discovery. Labels and availability can still vary by account/workspace. Login and consent remain operator-controlled. The legacy MCP-only mode cannot start a stopped conversation; the separately configured, opt-in [personal-chat controller](chatgpt-personal-control.md) implements an experimental UI transport with independent login and verification prerequisites. See [OpenAI's connection instructions](https://developers.openai.com/plugins/deploy/connect-chatgpt) and [OAuth requirements](https://developers.openai.com/plugins/build/auth).

Use **Riprendi un collegamento** after a reload. Only the worker ID is kept in session storage—never OAuth tokens or claim handles. Expired pairing windows can be reopened; **Rigenera istruzioni** prepares a bootstrap prompt without replacing the grant. It is not a safe replacement for the original open_id of an active run. Use the personal-controller **Riprendi ascolto** action for bounded resume with the handles already held by the chat. Re-pairing an already authorized worker requires confirmation. Errors retain the selected worker so a partial setup can be resumed without creating duplicates. The UI supports keyboard focus, live status announcements, mobile layout, and clipboard fallback.

If the gateway is disabled, the wizard shows a short operator setup panel for `https://gemrouter.example.com`. It does not edit `.env`, deploy, or restart the server. The code remains usable at other origins configured through `GEMROUTER_CHATGPT_PUBLIC_BASE_URL`.
...
```bash
GEMROUTER_BASE_URL=https://gemrouter.example.com \
GEMROUTER_API_KEY='app-key-for-this-worker' \
GEMROUTER_CHATGPT_ALIAS='chatgpt-research' \
pnpm smoke:chatgpt-mcp:live
```

A real live acceptance test requires operator login/consent and observation of the exact personal conversation, whether activated manually or using the optional verified controller: preserve an existing grant, bootstrap only when no active run remains, observe polling, send an app request, and verify that the real conversation returns it through MCP. Record that separately from the simulated smoke; an installed connector, issued grant, or successful remote smoke alone is not proof of a live ChatGPT completion. The remote script verifies the gateway response, not the worker's identity.
